...
if ( ! class_exists( 'WPSEO_Local_Storelocator' ) ) {
	...
	class WPSEO_Local_Storelocator {
		...
		private function get_locations( $nr_results, $check_outside_radius ) {
			...
			$radius           = ( ! empty( $_REQUEST['wpseo-sl-radius'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['wpseo-sl-radius'] ) ) : 99_999;
			...
			$coordinates = (object) [
				'lat' => isset( $_POST['wpseo-sl-lat'] ) ? (float) $_POST['wpseo-sl-lat'] : 0.0,
				'lng' => isset( $_POST['wpseo-sl-lng'] ) ? (float) $_POST['wpseo-sl-lng'] : 0.0,
			];
